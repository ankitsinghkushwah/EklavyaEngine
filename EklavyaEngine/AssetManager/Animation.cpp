...
{
  Animation::Animation(const std::string &assetName, int modelID,
                       const aiAnimation *animation, const aiScene *scene)
      : IAsset(EType::ANIMATION, assetName), m_ModelID(modelID)
  {
    ...
    for (int i = 0; i < size; i++)
      {
        ...
        m_Joints.push_back(Bone(channel->mNodeName.data,
  ...
  {
    auto iter =
        std::find_if(m_Joints.begin(), m_Joints.end(), [&](const Bone &joint) {
