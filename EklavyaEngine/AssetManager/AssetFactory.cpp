...
SHARED_TEXTURE AssetFactory::CreateTexture(std::string assetName,
                                           std::string ext, bool repeat) {
  SHARED_TEXTURE newTexture = std::make_shared<Texture2D>(assetName);
  uint32_t created = newTexture->CreateTexture(
      s_TexturesDirPath + assetName + "." + ext, repeat );
...
SHARED_SHADER AssetFactory::CreateShader(std::string assetName) {
  ...
  for (int type = 0; type < EShaderType::SHADER_TYPE_MAX; ++type) {
    ...
    shader->AddAndCompile(s_ShadersDirPath + assetName + suffix,
                          (EShaderType)type);
  }
  shader->Build();
  return shader;
...
SHARED_CUBEMAP AssetFactory::CreateCubemap(std::string folderName,
                                           std::string ext) {
...
SHARED_MODEL AssetFactory::CreateModel(std::string assetName, std::string ext,
                                       int modelID) {
...
SHARED_ANIMATION AssetFactory::CreateAnimation(std::string assetName,
                                               std::string ext, int modelID) {
  Assimp::Importer importer;
  std::string path = s_ModelsDirPath + assetName + ext;
  const aiScene *scene =
      importer.ReadFile(path,aiProcess_Triangulate | aiProcess_OptimizeGraph);
  if (!scene || !scene->mRootNode) {
    std::string error = std::string(importer.GetErrorString());
    return nullptr;
  }
  SHARED_ANIMATION newAnimation = std::make_shared<Animation>(
      assetName, modelID, scene->mAnimations[0], scene);
  return newAnimation;
}
