...
SHARED_TEXTURE AssetManager::LoadTexture(std::string assetName, std::string ext,
                                         bool repeat) {
...
SHARED_CUBEMAP AssetManager::LoadCubemap(std::string folderName,
                                         std::string ext) {
...
SHARED_MODEL AssetManager::LoadModel(std::string folderName, std::string ext,
                                     int modelID) {
...
SHARED_ANIMATION AssetManager::LoadAnimation(std::string animName,
                                             std::string ext, int modelID) {
  SHARED_ANIMATION newAsset =
      AssetFactory::CreateAnimation(animName, ext, modelID);
...
void AssetManager::RemoveAsset(EType type, const std::string &name) {
  ...
  auto iter = std::find_if(mAssetMap[type].begin(), listEnd,
