...
public:
  AssetManager();
  ~AssetManager();
  SHARED_TEXTURE LoadTexture(std::string assetName, std::string ext,
                             bool repeat = false);
  ...
  SHARED_ANIMATION LoadAnimation(std::string animName, std::string ext,
                                 int modelID);
  ...
  std::shared_ptr<Type> GetAsset(const std::string &name) {
    ...
    auto iter = std::find_if(mAssetMap[type].begin(), listEnd,
                             [&](std::shared_ptr<Asset::IAsset> asset) {
