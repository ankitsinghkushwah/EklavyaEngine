...
void GLModel::Load(const std::string &path)
{
  ...
  const aiScene   *scene = importer.ReadFile(
      path,
      aiProcess_Triangulate | aiProcess_GenBoundingBoxes | aiProcess_FlipUVs);
...
std::shared_ptr<ModelNode> GLModel::ProcessNode(aiNode        &parentNode,
                                                const aiScene &scene,
  ...
  for (unsigned int i = 0; i < parentNode.mNumChildren; i++)
    {
      ...
      std::shared_ptr<ModelNode> childNode =
          ProcessNode(*childAssimpNode, scene, childTransform);
...
{
  auto loadTexture = [&](aiTextureType texType, const aiMaterial *material,
                         bool firstIndex = true) -> SHARED_TEXTURE {
  ...
  for (uint32_t mat_index = 0; mat_index < pScene->mNumMaterials; ++mat_index)
    {
      ...
      matInfo.mRoughMap =
          loadTexture(aiTextureType_METALNESS, material); // roughness
...
void GLModel::SetVertexBoneData(MeshVAOData &meshData, int vertexIdx,
...
void GLModel::ExtractBoneWeightForVertices(MeshVAOData           &meshData,
                                           std::vector<BoneInfo> &meshBones,
                                           const aiMesh          &mesh,
                                           const aiScene         &scene)
{
