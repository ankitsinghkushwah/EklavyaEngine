...
namespace Eklavya::Asset
{
  ...
  private:
    std::shared_ptr<ModelNode> ProcessNode(aiNode        &parentNode,
                                           const aiScene &scene,
                                           glm::mat4     &parentTransform);
    GLMesh *ExtractMeshData(const aiMesh &mesh, const aiScene &scene);
    void    LoadMaterials(const aiScene *scene);
    void    SetVertexBoneData(MeshVAOData &meshData, int vertexIdx, int boneID,
                              float weight);
    void    ExtractBoneWeightForVertices(MeshVAOData           &meshData,
