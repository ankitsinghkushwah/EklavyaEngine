...
ShaderProgram::ShaderProgram(const std::string &assetName)
    : IAsset(EType::SHADER, assetName), m_IsReadyToUse(false),
      m_ErrorInfo("N/A") {
...
bool ShaderProgram::AddAndCompile(const std::string &shaderPath,
                                  EShaderType type) {
...
bool ShaderProgram::ErrorOccured(GLuint id, bool linking) {
  ...
  if (!success) {
    ...
    m_ErrorInfo =
        prefix + " " + m_Name + " :: Error :: " + std::string(errInfo);
    printf(m_ErrorInfo.c_str());
    printf("\n");
    return true;
  }
...
void ShaderProgram::SetVec2(const std::string &pName,
                            const glm::vec2 &pVector) {
  glUniform2f(glGetUniformLocation(m_ID, pName.c_str()), pVector.x, pVector.y);
}

void ShaderProgram::SetVec3(const std::string &pName,
                            const glm::vec3 &pVector) {
  glUniform3f(glGetUniformLocation(m_ID, pName.c_str()), pVector.x, pVector.y,
              pVector.z);
}

void ShaderProgram::SetVec4(const std::string &pName,
                            const glm::vec4 &pVector) {
  glUniform4f(glGetUniformLocation(m_ID, pName.c_str()), pVector.x, pVector.y,
              pVector.z, pVector.w);
}

void ShaderProgram::SetMat4(const std::string &pName, const glm::mat4 &pMat) {
  int id = glGetUniformLocation(m_ID, pName.c_str());
  glUniformMatrix4fv(glGetUniformLocation(m_ID, pName.c_str()), 1, GL_FALSE,
                     glm::value_ptr(pMat));
}

void ShaderProgram::SetNMat4(const std::string &pName,
                             std::vector<glm::mat4> transforms, int size) {
  int id = glGetUniformLocation(m_ID, pName.c_str());
