...
namespace Eklavya
{
	...
	void VertexArrayObject::SetBoneWeightPtr(int startOffset, int strideOffset)
	{
		...
		glVertexAttribPointer(3, 4, GL_FLOAT, GL_FALSE, strideOffset, (void *) startOffset);
		Unbind();
	}
	...
	void VertexArrayObject::SetBoneIdsPtr(int startOffset, int strideOffset)
	{
		...
		glVertexAttribPointer(4, 4, GL_FLOAT, GL_FALSE, strideOffset, (void *) startOffset);
		Unbind();
	}

	// STATICS
	void VertexArrayObject::SetupVAOForSphere(VertexArrayObject &vao, glm::vec3 &min, glm::vec3 &max)
	{
		float radius = 1.0f;
		...
		{
			stackAngle = 3.1415926535 / 2 - i * stackStep; // starting from pi/2 to -pi/2
		...
		vao.Create(buffer.data(), bufferSize, indices.data(), sizeof(int) * indices.size());
		vao.SetPosPtr(3, 0, 0);
		vao.SetNormalPtr(sizeof(float) * vertices.size(), 0);
		vao.SetTexPtr(sizeof(float) * (vertices.size() + normals.size()), 0);
	}

	void VertexArrayObject::SetupVAOForBox(VertexArrayObject &vao, glm::vec3 &min, glm::vec3 &max)
	{
		float vertices[] = {
			// Back face
			-0.5f,
