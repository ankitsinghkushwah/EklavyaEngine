...
TransformComponent::TransformComponent()
    : EkComponent("Transform"), mRotation(), mScale(1.0f), mPosition(0.0f),
      mLocalPosition(0.0f), mLocalScale(1.0f), mLocalRotation(), mWorld(1.0f),
      mLocal(1.0f), mParent(1.0f),

      mForward(0.0f, 0.0f, -1.0f), mRight(1.0f, 0.0f, 0.0f),
      mUp(0.0f, 1.0f, 0.0f)
...
void TransformComponent::SetRotation(const glm::vec3 &pEulerAngles) {
  glm::vec3 rads = {glm::radians(pEulerAngles.x), glm::radians(pEulerAngles.y),
                    glm::radians(pEulerAngles.z)};
...
void TransformComponent::UpdateBasis() {
  ...
  mForward =
      glm::normalize(glm::vec3(mWorld[2][0], mWorld[2][1], mWorld[2][2]));
}
...
const glm::mat4 &TransformComponent::GetRotationMatrix() const {
