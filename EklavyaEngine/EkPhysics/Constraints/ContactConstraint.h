...
struct CollisionPointInfo {
  ...
  CollisionPointInfo(const glm::vec3 &loc, const glm::vec3 &norm, float pen)
      : location(loc), normal(norm), peneteration(pen) {}
  glm::vec3 location;
  glm::vec3 normal;
  float peneteration;
};
...
public:
  ...
  void AddCollisionPoint(glm::vec3 point, glm::vec3 collisionNormal,
                         float peneteration) {
