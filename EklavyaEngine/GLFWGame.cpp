...
bool GLWindowContext::Create(const std::string &name, int majorVersion,
                             int minorVersion, int width, int height,
                             bool fullScreen)
{
  m_Name = name;
  m_X = m_Y = 0;
  ...
  m_Window =
      glfwCreateWindow(width, height, name.c_str(),
...
void GLWindowContext::SwapBuffers() { glfwSwapBuffers(m_Window); }
...
{
  ImGui::Begin(
      "STATS"); // Create a window called "Hello, world!" and append into it.
  ImGui::Text("Application average %.3f ms/frame (%.1f FPS)",
              1000.0f / ImGui::GetIO().Framerate, ImGui::GetIO().Framerate);
  ImGui::End();
}

bool GLFWGame::Initialize(const std::string &pName, int pWndWidth,
                          int pWndHeight, bool pFullScreen)
{
  if (!glfwInit())
    {
  ...
  bool success =
      m_CurrentContext->Create(pName, 4, 3, pWndWidth, pWndHeight, pFullScreen);
  if (!success)
    return false;
...
{
  glfwSetInputMode(m_CurrentContext->GetWindow(), GLFW_CURSOR,
      ...
      if (!glfwWindowShouldClose(m_CurrentContext->GetWindow()))
        {
          ...
          glfwGetFramebufferSize(m_CurrentContext->GetWindow(), &display_w,
                                 &display_h);
...
void CallbackManager::KeyCallback(GLFWwindow *window, int key, int scancode,
                                  int action, int mods)
{
  if (mApp)
...
void CallbackManager::WindowResizeCallback(GLFWwindow *window, int width,
                                           int height)
{
  if (mApp)
...
void CallbackManager::MouseButtonCallback(GLFWwindow *window, int key,
                                          int action, int mods)
{
  if (mApp)
...
void CallbackManager::CursorPositionCallback(GLFWwindow *window, double xPos,
                                             double yPos)
{
  if (mApp)
