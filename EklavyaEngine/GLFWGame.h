...
public:
  GLWindowContext();
  ~GLWindowContext();
  bool Create(const std::string &name, int majorVersion, int minorVersion,
...
public:
  GLFWGame();
  virtual ~GLFWGame();
  virtual bool Initialize(const std::string &pName, int pWindowWidth,
                          int pWindowHeight, bool pFullScreen);
