...
// SHADOW PASS
ShadowPassFB::ShadowPassFB(int depthPrecision, int width, int height) : mDepthSize(depthPrecision), mWidth(width), mHeight(height)
{
}
...
bool ShadowPassFB::Setup()
{
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, mDepthOutput, 0);
...
GeneralFB::GeneralFB(int depthSize, int colorChannels, float width, float height) : mChannels(colorChannels), mDepthSize(depthSize), mWidth(width), mHeight(height)
{
}
...
bool GeneralFB::Setup()
{
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, mColorOutput, 0);
