...
namespace Eklavya::Renderer
{
  GLRenderer::GLRenderer(const GLWindowContext &context)
      : mContext(context), mOutputModel(1.0f)
  {
    for (int rp = 0; rp < mRenderPasses.size(); ++rp)
      {
        switch (rp)
          ...
          case ERenderPass::SHADOW_PASS:
            mRenderPasses[rp] =
                std::make_unique<ShadowMapPass>(64, 1024.0f * 4, 1024.0f * 4);
            break;
          case ERenderPass::MAIN_PASS:
            mRenderPasses[rp] = std::make_unique<MainPass>(
                32, 4, mContext.GetWidth(), mContext.GetHeight());
            break;
          }
  ...
  {
    SHARED_SHADER skyShader =
        AssetManager::GetInstance().GetAsset<ShaderProgram>("skybox");
    ...
    SHARED_SHADER brdf =
        AssetManager::GetInstance().GetAsset<ShaderProgram>("standard_brdf");
    SHARED_PBR_MATERIAL pbrMat = std::make_shared<PBRMaterial>(brdf);
    AddMaterialToState(Renderer::ERenderGroup::ACTOR, pbrMat);
    mMainOutputShader =
        AssetManager::GetInstance().GetAsset<ShaderProgram>("main_output");
  }
  ...
  void GLRenderer::AddMaterialToState(ERenderGroup    group,
                                      SHARED_MATERIAL material)
  {
    mGroupMaterials[group] = material;
  }

  void GLRenderer::AddActor(SHARED_RENDERCOMPONENT renderComponent)
  {
  ...
  void GLRenderer::PostPassesResult()
  {
    ...
    GLint texture =
        mRenderPasses[ERenderPass::MAIN_PASS]->mFramebuffer->ColorOutput();
  ...
  void GLRenderer::SetShaderDataForActor(
      const SHARED_RENDERCOMPONENT &renderComponent, SHARED_SHADER &shader)
  {
    if (renderComponent->mHasBones)
      {
        shader->SetInt("bApplyAnimation", 1);
        std::vector<glm::mat4> &transforms = renderComponent->boneTransforms;
  ...
  void
  GLRenderer::RenderInternal(const std::vector<SHARED_RENDERCOMPONENT> &actors,
                             SHARED_SHADER &shader, SHARED_MATERIAL material)
  {
    for (SHARED_RENDERCOMPONENT renderComponent : actors)
      {
        VertexArrayObject &vao = renderComponent->GetMesh().VAO();
        vao.Bind();
        glm::mat4 &model = renderComponent->mWorldMatrix;
        if (material != nullptr)
          {
        ...
        if (vao.HasSharedVertices())
          glDrawElements(GL_TRIANGLES, vao.GetIndicesSize(), GL_UNSIGNED_INT,
                         0);
        else
