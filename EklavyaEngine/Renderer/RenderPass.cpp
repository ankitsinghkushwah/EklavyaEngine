...
ShadowMapPass::ShadowMapPass(int depthPrecision, int width, int height)
    : IRenderPass(ERenderPass::SHADOW_PASS)
    , mDepthPrecision(depthPrecision)
    , mWidth(width)
    , mHeight(height)
{
}
...
{
	mShader = AssetManager::GetInstance().GetAsset<Eklavya::Asset::ShaderProgram>("shadow_map");
	mFramebuffer = std::make_unique<ShadowPassFB>(mDepthPrecision, mWidth, mHeight);
	mFramebuffer->Setup();
	return false;
}

void ShadowMapPass::TryAddingActor(const RenderComponent& renderComponent)
{
...
void ShadowMapPass::Run(GLRenderer& renderer, const EkScene& scene)
{
	...
	float     extents = 1500;
	glm::mat4 projection = glm::ortho(-extents, extents, -extents, extents, -far_plane, far_plane);
...
MainPass::MainPass(int depth, int colorChannels, int width, int height) : IRenderPass(ERenderPass::MAIN_PASS), mDepthPrecision(depth), mWidth(width), mHeight(height)
{
}
...
bool MainPass::Init()
{
	mFramebuffer = std::make_unique<GeneralFB>(mDepthPrecision, 4, mWidth, mHeight);
	mFramebuffer->Setup();
	return true;
}

void MainPass::TryAddingActor(const RenderComponent& renderComponent)
{
	if (renderComponent.mInsideFrustum || renderComponent.mRenderGroup == ERenderGroup::SKYBOX)
	{
...
void MainPass::Run(GLRenderer& renderer, const EkScene& scene)
{
	...
	const ShadowMapPass* shadowPass = (ShadowMapPass*)renderer.mRenderPasses[0].get();
