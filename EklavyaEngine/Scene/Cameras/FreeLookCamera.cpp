...
namespace Eklavya
{
  FreeLookCamera::FreeLookCamera(CameraParams options)
      : ICamera(options), mSenstivity(.005), mSpeed(100.0f)
  {
  ...
  {
    mView = glm::lookAt(mPosition, mPosition + mForward,
  ...
  void FreeLookCamera::MoveForController(float dt)
  {
    ...
    float rightY =
        -InputHandler::GetInstance()->GetAxis(GLFW_GAMEPAD_AXIS_RIGHT_Y);

    float speed = mSpeed * dt;

    mPosition +=
        rightX * speed * glm::cross(glm::vec3(0.0f, 1.0f, 0.0f), mForward);
