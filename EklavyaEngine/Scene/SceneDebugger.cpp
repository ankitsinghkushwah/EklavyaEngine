...
namespace Eklavya
{
  ...
  void SceneDebugger::TraverseToDebugDraw(SHARED_ACTOR actor)
  {
    ...
    SHARED_EKBODY body = actor->GetComponent<Physics::EkBody>("EkBody");
    if (body && body->mShowCollider)
      {
        ...
        if (body->GetCollider()->GetType() == Physics::EColliderType::BOX)
          {
            glm::vec3 extents = std::dynamic_pointer_cast<Physics::BoxCollider>(
                                    body->GetCollider())
                                    ->GetHalfSize() *
                                2.0f;
        ...
        if (body->GetCollider()->GetType() == Physics::EColliderType::SPHERE)
          {
            float radius = std::dynamic_pointer_cast<Physics::SphereCollider>(
                               body->GetCollider())
                               ->GetRadius();
  ...
  void SceneDebugger::OnKeyAction(int key, int action)
  {
    if (key == GLFW_KEY_P && action == GLFW_PRESS)
      {
        mDebug = !mDebug;
        mScene.mDirector->HideMouse(mDebug == false);
        mScene.mFreeLookCamera->SetEnabled(mDebug == false);
      }
  }
  ...
  bool SceneDebugger::CanTraverse(SHARED_ACTOR &actor)
  {
    ...
    if (mFilterFlags[ESceneDebugFlags::ANIMATED] &&
        actor->GetComponent<Animator>("AnimationComponent") != nullptr)
      {
        return true;
      }
    return false;
