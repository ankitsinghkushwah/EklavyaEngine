...
void SpringFollowCamera::Init(float fov, float ratio, float nearDist,
                              float farDist) {
  ...
  mView =
      glm::lookAt(mPosition, mPosition + mFront, glm::vec3(0.0f, 1.0f, 0.0f));
}
...
void SpringFollowCamera::UpdateCamera(float dt) {
  ...
  glm::vec3 localArmsLength =
      glm::vec3(mTarget->Rotation() * glm::vec4(mArmLengths, 1.0f));
